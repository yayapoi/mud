...
{
    //qDebug() << "TcpServerControl::sockDisConnectSlot  prot--"<<prot;
...
{
    //qDebug() << "TcpServerControl::getSendMessage  name--"<<name;
...
void TcpServerControl::incomingConnection(qintptr socketDescriptor)
{
    ...
    connect(tcpTemp,&TcpConnect::getMessage,this,&TcpServerControl::getMessage,Qt::QueuedConnection);//接受到数据
    connect(tcpTemp,&TcpConnect::sockDisConnect,this,&TcpServerControl::sockDisConnectSlot,Qt::QueuedConnection);//断开连接的处理，从列表移除，并释放断开的Tcpsocket
    connect(tcpTemp,&TcpConnect::sockDisConnect,thread,&QThread::quit,Qt::QueuedConnection);//断开连接时线程退出
    connect(tcpTemp,&TcpConnect::connectClient,this,&TcpServerControl::connectClient,Qt::QueuedConnection);//发送连接信号
    connect(this,&TcpServerControl::sendMessage,tcpTemp,&TcpConnect::sendMessage,Qt::QueuedConnection);//发送数据
