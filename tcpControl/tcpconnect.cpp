...
{
    //qDebug() << "TcpConnect::connectToHost";
    ...
    connect(tcpSocket,&QTcpSocket::disconnected,[&](){
        //qDebug() << "QTcpSocket::disconnected thisPort--"<<thisPort;
        emit sockDisConnect(socketDescriptor11,"",thisPort);
    });
    thisPort=tcpSocket->peerPort();
    emit connectClient(tcpSocket->socketDescriptor(),tcpSocket->peerAddress().toString(),tcpSocket->peerPort());
    //qDebug() << "TcpConnect::connectToHost  peerPort--"<<tcpSocket->peerPort();
}
...
void TcpConnect::sendMessage(int socketPort, QString lei, QString name, QStringList regList)
{
    if(socketPort==thisPort)
    {
        ...
        tcpSocket->flush();
        //qDebug() << "TcpConnect::sendMessage  backStr--"<<backStr;
    }
}
